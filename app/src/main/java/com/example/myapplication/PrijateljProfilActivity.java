...
public class PrijateljProfilActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db = new DatabaseHelper(PrijateljProfilActivity.this);
        ...
        slike = new ArrayList<>();
        getVals();
        ObjaveAdapter objaveAdapter = new ObjaveAdapter(this, film_id, ime, zanr, godina, opis,ime_k,id_k,slike, PrijateljProfilActivity.this);
        rv.setAdapter(objaveAdapter);
        rv.setLayoutManager(new LinearLayoutManager(PrijateljProfilActivity.this));
    ...
    void getVals(){
        ...
        String idP = temp.getStringExtra("idP");
        query = "SELECT f.ID,f.IME,f.GODINA,f.OPIS,f.ZANR,u.ID,u.USERNAME,f.SLIKA" +
                " FROM Filmovi f, User u, USER_FILM uf WHERE  " +
                "uf.U_ID=u.ID AND uf.F_ID=f.ID AND u.ID = "+idP;

        Cursor cursor = db.execRawQuery(query);
        if (cursor.getCount() == 0) {
            Toast.makeText(PrijateljProfilActivity.this, "Nema podataka", Toast.LENGTH_LONG).show();
        } else {
