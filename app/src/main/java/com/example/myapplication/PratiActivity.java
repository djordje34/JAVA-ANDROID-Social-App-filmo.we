...
public class PratiActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rv = findViewById(R.id.rv_zaprati);
        pretraga = findViewById(R.id.buttonPretrazi);
        sta = findViewById(R.id.editTextPretraga);
        db = new DatabaseHelper(PratiActivity.this);
        id = new ArrayList<>();
        un = new ArrayList<>();
        br_f = new ArrayList<>();

        getVals();
        korisnikAdapter = new KorisnikAdapter(this, id, un, br_f, PratiActivity.this,0);
        rv.setAdapter(korisnikAdapter);
        rv.setLayoutManager(new LinearLayoutManager(PratiActivity.this));
        ...
        pretraga.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                i = new Intent(PratiActivity.this, PratiActivity.class);//.getText().toString();
    ...
    void getVals(){
        ...
        if(temp.hasExtra("pretraga")){
            ...
            query = "(ID!="+temp.getStringExtra("id")+"AND USERNAME LIKE '%"+pretraga+"%') AND ID NOT IN (SELECT U_ID2 FROM Korisnik_Prati WHERE U_ID1 = "+temp.getStringExtra("id")+"))";
        }
        else{
            query = " (ID!="+temp.getStringExtra("id")+" AND ID NOT IN (SELECT U_ID2 FROM Korisnik_Prati WHERE U_ID1 = "+temp.getStringExtra("id")+"))";
        }

        Cursor cursor = db.getData("User",query);
        if (cursor.getCount() == 0) {
            Toast.makeText(PratiActivity.this, "Nema podataka", Toast.LENGTH_LONG).show();
        } else {
