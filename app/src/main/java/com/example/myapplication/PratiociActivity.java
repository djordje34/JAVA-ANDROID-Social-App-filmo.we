...
public class PratiociActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db = new DatabaseHelper(PratiociActivity.this);
        id = new ArrayList<>();
        un = new ArrayList<>();
        br_f = new ArrayList<>();

        getVals();
        korisnikAdapter = new KorisnikAdapter(this, id, un, br_f, PratiociActivity.this,2);
        rv.setAdapter(korisnikAdapter);
        rv.setLayoutManager(new LinearLayoutManager(PratiociActivity.this));
        ...
        pretraga.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                i = new Intent(PratiociActivity.this, PratiociActivity.class);//.getText().toString();
    ...
    void getVals() {
        ...
        if (temp.hasExtra("pretraga")) {
            ...
            query = "(ID!='" + temp.getStringExtra("id") + "' AND USERNAME LIKE '%" + pretraga + "%') AND ID IN (SELECT U_ID1 FROM Korisnik_Prati WHERE U_ID2 = '" + temp.getStringExtra("id") + "')";
        } else {
            query = " (ID!= " + temp.getStringExtra("id") + " AND ID IN (SELECT U_ID1 FROM Korisnik_Prati WHERE U_ID2 = " + temp.getStringExtra("id") + "))";
        }

        Cursor cursor = db.getData("User", query);
        if (cursor.getCount() == 0) {
            Toast.makeText(PratiociActivity.this, "Nema podataka", Toast.LENGTH_LONG).show();
        } else {
